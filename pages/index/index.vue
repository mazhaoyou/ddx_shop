<template>
		...
		<view class="seckill-section m-t">
			...
			<scroll-view class="floor-list" scroll-x>
				<view class="scoll-wrapper">
					<view 
						v-for="(item, index) in newGoodList" :key="item.id"
						class="floor-item"
						@click="navToDetailPage(item)"
					>
						<image :src="item.list_pic_url" mode="aspectFill"></image>
	...
	<!-- 	<view class="f-header m-t">
			...
			<swiper class="g-swiper" :duration="500">
				...
				>
					...
					<view class="g-item right">
						<image :src="brandList[index+1].list_pic_url" mode="aspectFill"></image>
						<view class="t-box">
							<text class="title clamp">{{brandList[index+1].name}}</text>
							<view class="price-box">
								<text class="price">￥{{brandList[index+1].floor_price}}</text> 
								<text class="m-price">￥188</text> 
							</view>
							<view class="pro-box">
							  	<view class="progress-box">
		...
		<!-- 分类推荐楼层结束 -->
		<!-- 猜你喜欢开始 -->
		<block v-for="(item,index) in newCategoryList" :key="item.id">
			<view class="f-header m-t">
				<image src="/static/temp/h1.png"></image>
				<view class="tit-box">
			...
			<view class="guess-section">
				<view 
					v-for="(item1, index) in item.goodsList" :key="item1.id"
					class="guess-item"
					@click="navToDetailPage(item1)"
				>
					<view class="image-wrapper">
						<image :src="item1.list_pic_url" mode="aspectFill"></image>
					</view>
					<text class="title clamp">{{item1.name}}</text>
			...
			getNewGoods(){
				this.$http.newGoods()
					.then(res => {
						res = res.data;
						if(res.code == '0001') {
							this.newGoodList = res.data.newGoods;
						} else {
							this.$api.msg(res.message);
						}
					})
